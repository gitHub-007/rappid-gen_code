...
public class GeneratorControl {
    private static Logger logger = LoggerFactory.getLogger(GeneratorControl.class);
    ...
    public GeneratorControl(GeneratorProperties generatorProperties) {
        this.isOverride = generatorProperties.getBoolean(GG_IS_OVERRIDE);
    ...
    public void generateFile(String outputFile, String content, boolean append) {
        try {
            ...
            if (deleteGeneratedFile) {
                logger.info("[delete gg.generateFile()] file:{} by template:{}", realOutputFile, getSourceFile());
                new File(realOutputFile).delete();
            } else {
                File file = new File(realOutputFile);
                FileHelper.parentMkdir(file);
                logger.info("[gg.generateFile()] outputFile:{} append:{} by {} template:", realOutputFile, append,
                            getSourceFile());
                IOHelper.saveFile(file, content, getOutputEncoding(), append);
            }
        } catch (Exception e) {
            logger.warn("gg.generateFile() occer error,outputFile:{} caused by:{}", outputFile, e, e);
    ...
    public String getInputProperty(String key, String message) throws IOException {
        ...
        if (v == null) {
            if (SystemHelper.isWindowsOS) {
                v = JOptionPane.showInputDialog(null, message, "template:" + getSourceFileName(), JOptionPane
                        .OK_OPTION);
            } else {
