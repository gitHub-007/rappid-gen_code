...
public class Generator {
    ...
    private String includes;         // 需要处理的模板，使用逗号分隔符,示例值:
    // java_src/**,java_test/**
    private String excludes ;         // 不需要处理的模板，使用逗号分隔符,示例值:
    // java_src/**,java_test/**
    ...
    public Generator( GeneratorProperties generatorProperties) {
        ...
        this.removeExtensions = generatorProperties.getProperty(GENERATOR_REMOVE_EXTENSIONS);
        this.includes = generatorProperties.getProperty(GENERATOR_INCLUDES);         // 需要处理的模板，使用逗号分隔符,示例值:
        this.excludes = generatorProperties.getProperty(GENERATOR_EXCLUDES);         // 不需要处理的模板，使用逗号分隔符,示例值:
        this.sourceEncoding = generatorProperties.getProperty(GENERATOR_SOURCE_ENCODING);
        this.outputEncoding = generatorProperties.getProperty(GENERATOR_OUTPUT_ENCODING);
    }
    ...
    public void addClassPathTemplateRootDir(String file) {
        addTemplateRootDir(String.format("classpath:%s",file));
    }
    ...
    public void deleteOutRootDir() throws IOException {
        ...
        logger.warn("[delete dir]    {}", getOutRootDir());
//        FileHelper.deleteDirectory(new File(getOutRootDir()));
        FileHelper.deleteDirectory(Paths.get(getOutRootDir()).toAbsolutePath().toFile());
    ...
    private void processTemplateRootDirs(Map templateModel, Map filePathModel, boolean isDelete) throws Exception {
        ...
        logger.debug("******* Template reference variables *********\n{}", templateModel);
        ...
        GeneratorException ge = new GeneratorException("generator occer error, Generator BeanInfo:" + BeanHelper
                .describe(this));
        ...
        for (int i = 0; i < processedTemplateRootDirs.size(); i++) {
            ...
            List<Exception> exceptions = scanTemplatesAndProcess(templateRootDir, processedTemplateRootDirs,
                                                                 templateModel, filePathModel, isDelete);
     ...
     */
    private List<Exception> scanTemplatesAndProcess(File templateRootDir, List<File> templateRootDirs, Map
            templateModel, Map filePathModel, boolean isDelete) throws Exception {
        if (templateRootDir == null) throw new IllegalStateException("'templateRootDir' must be not null");
        logger.warn("-------------------load template from templateRootDir = {} outRootDir:{}", templateRootDir
                .getAbsolutePath(), new File(outRootDir).getAbsolutePath());
            ...
            try {
                if (isDelete) {
                    ...
                    new TemplateProcessor(templateRootDirs).executeGenerate(templateRootDir, templateModel,
                                                                            filePathModel, srcFile);
                    logger.debug("genereate by tempate cost time:" + (System.currentTimeMillis() - start) + "ms");
                }
            } catch (Exception e) {
                if (ignoreTemplateGenerateException) {
                    logger.warn("iggnore generate error,template is:{}", srcFile, e);
                    exceptions.add(e);
                } else {
                    throw e;
                }
    ...
    private class TemplateProcessor {
        private GeneratorControl gg = new GeneratorControl(Generator.this.generatorProperties);
        ...
        private void executeGenerate(File templateRootDir, Map templateModel, Map filePathModel, File srcFile) throws
                SQLException, IOException, TemplateException {
            ...
            if (isCopyBinaryFile && FileHelper.isBinaryFile(srcFile)) {
                ...
                logger.info("[copy binary file by extention] from:{} =>{}", srcFile, outputFile);
            ...
            try {
                ...
                throw new RuntimeException("generate oucur error,templateFile is:" + templateFile + " => " + gg
                        .getOutputFile() + " cause:" + e, e);
            }
        }

        private void executeDelete(File templateRootDir, Map templateModel, Map filePathModel, File srcFile) throws
                SQLException, IOException, TemplateException {
            String templateFile = FileHelper.getRelativePath(templateRootDir, srcFile);
            if (GeneratorHelper.isIgnoreTemplateProcess(srcFile, templateFile, includes, excludes)) {
                return;
        ...
        private String proceeForOutputFilepath(Map filePathModel, String templateFile) throws IOException {
            ...
            if ((testExpressionIndex = templateFile.indexOf('@')) != -1) {
                ...
                if (!"true".equals(String.valueOf(expressionValue))) {
                    logger.info("[not-generate]\t test expression '@{}' is false,template: {}", testExpressionKey,
                                templateFile);
                    return null;
                }
            }
        ...
        private void generateNewFileOrInsertIntoFile(String templateFile, String outputFilepath, Map templateModel)
            ...
            if (absoluteOutputFilePath.exists()) {
                ...
                if (GeneratorHelper.isFoundInsertLocation(gg, template, templateModel, absoluteOutputFilePath,
                    ...
                    IOHelper.saveFile(absoluteOutputFilePath, newFileContentCollector.toString(), gg
                            .getOutputEncoding());
                    return;
                }
            }
            ...
            if (absoluteOutputFilePath.exists() && !gg.isOverride()) {
                logger.info("[not generate]\t by gg.isOverride()=false and outputFile exist:{}", outputFilepath);
                return;
            }

            if (absoluteOutputFilePath.exists()) {
                logger.info("[override]\t template:{} ==>{}", templateFile, outputFilepath);
            } else {
                logger.info("[override]\t template:{} ==>{}", templateFile, outputFilepath);
            }
    ...
    static class GeneratorHelper {
        ...
        public static boolean isIgnoreTemplateProcess(File srcFile, String templateFile, String includes, String
                excludes) {
            if (srcFile.isDirectory() || srcFile.isHidden()) return true;
            if (templateFile.trim().equals("")) return true;
        ...
        private static boolean isFoundInsertLocation(GeneratorControl gg, Template template, Map model, File
                outputFile, StringWriter newFileContent) throws IOException, TemplateException {
        ...
        public static Configuration newFreeMarkerConfiguration(List<File> templateRootDirs, String defaultEncoding,
            ...
            //			List<String> availableAutoInclude = FreemarkerHelper.getAvailableAutoInclude(conf, Arrays
            // .asList("macro.include",autoIncludes));
            //			conf.setAutoIncludes(availableAutoInclude);
            //			GLogger.info("[set Freemarker.autoIncludes]"+availableAutoInclude+" for
            ...
            conf.setAutoIncludes(availableAutoInclude);
            logger.trace("set Freemarker.autoIncludes:{} for templateName:{} autoIncludes:{}", availableAutoInclude,
