...
public class JavaMethod {
    private static Logger logger = LoggerFactory.getLogger(JavaMethod.class);
    Method method;
    ...
    public boolean isPropertyMethod() {
        if (getMethodName().startsWith("set") || getMethodName().startsWith("get") || (getMethodName().startsWith
                ("is") && getReturnType().isBooleanType())) {
            return true;
        }
        return false;
    }
    ...
    public List<FieldMethodInvocation> getFieldMethodInvocationSequences() {
        if (StringHelper.isNotBlank(clazz.getMavenJavaSourceFileContent())) {
            try {
                JavaMethodInvokeSequencesParser cmd = new JavaMethodInvokeSequencesParser(this, clazz
                        .getMavenJavaSourceFileContent());
                cmd.execute();
                return cmd.getMethodInvokeSequences();
            } catch (Exception e) {
                logger.warn("getFieldMethodInvocationSequences() occer error on method:{}", method.toString(), e);
