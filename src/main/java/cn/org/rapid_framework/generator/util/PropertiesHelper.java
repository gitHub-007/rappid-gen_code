...
public class PropertiesHelper {
    ...
    public static String[] loadAllPropertiesFromClassLoader(Properties properties, String... resourceNames) throws
            IOException {
...
//            }
//        }
        String resourceName = Arrays.stream(resourceNames).filter(fileName -> fileName.startsWith("default")).collect
                (Collectors.toList()).get(0);
        successLoadProperties.addAll(loadCustomProperties(properties, resourceName));
        List<String> fileNames = Arrays.stream(resourceNames).filter(fileName -> !fileName.startsWith("default")).sorted()
                .collect(Collectors.toList());
        for (String fileName : fileNames) {
            successLoadProperties.addAll(loadCustomProperties(properties, fileName));
        }
        return (String[]) successLoadProperties.toArray(new String[0]);
    }

    private static List<String> loadCustomProperties(Properties properties, String resourceName) throws IOException {
        Enumeration urls = ClassHelper.getDefaultClassLoader().getResources(resourceName);
        List<String> successLoadProperties = new ArrayList();
        while (urls.hasMoreElements()) {
            URL url = (URL) urls.nextElement();
    ...
    private static String resolveProperty(String v, Properties props) {
        PropertyPlaceholderConfigurerResolver propertyPlaceholderConfigurerResolver = new
