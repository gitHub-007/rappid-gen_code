...
public class JavaPrimitiveTypeMapping {
    ...
    static Map<String, Class> primitive2wraper = new HashMap();
    ...
    static {
        ...
        wraper2primitive.forEach((key, value) -> {
            try {
                primitive2wraper.put(value, Class.forName(String.format("java.lang.%s", key)));
            } catch (Exception e) {
                e.printStackTrace();
            }
        });
    ...
    public static String getPrimitiveType(String clazz) {
        String className = StringHelper.getJavaClassSimpleName(clazz);
        String result = wraper2primitive.getOrDefault(className, null);
        return result == null ? clazz : result;
    }

    public static String getWrapperTypeOrNull(String clazz) {
        String result = primitive2wraper.get(clazz).getSimpleName();
        return result;
    }

    public static String getWrapperType(String clazz) {
        Class result = primitive2wraper.getOrDefault(clazz, null);
        return result == null ? clazz : result.getSimpleName();
    }

    public static Class getWrapperClass(Class clazz) {
        Class result = primitive2wraper.getOrDefault(clazz.getSimpleName(), null);
        return result == null ? clazz : result;
    }
